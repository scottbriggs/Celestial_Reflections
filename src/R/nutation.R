...
nutationAngles <- function(jd)
{
  ...
  chebyshev_nut <- matrix(0.0, nrow=DE441NUMCOEFFNUTATION, ncol=1)
  chebyshev_nut[1] <- 1.0
  chebyshev_nut[2] <- x
  ...
  for (v in seq(from = DE441NUMCOEFFNUTATION, to = 1, by = -1)){
    nut_ang[1,1] <- nut_ang[1,1] + (chebyshev_nut[v] * df_nutation[v])
    nut_ang[1,2] <- nut_ang[1,2] + (chebyshev_nut[v] * df_nutation[v+DE441NUMCOEFFNUTATION])
  }
